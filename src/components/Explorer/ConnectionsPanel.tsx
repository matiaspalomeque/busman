...
const CP_ENV_ORDER = ["prod", "staging", "dev"];

function buildConnectionGroups(items: Connection[]) {
  ...
  return [
    ...CP_ENV_ORDER.filter((e) => buckets[e]?.length).map((e) => ({ key: e, items: buckets[e]! })),
...
export function ConnectionsPanel() {
  ...
  const groups = buildConnectionGroups(filtered);
  const showGroupHeaders = !search && groups.length > 1;

  const renderConnectionItem = (c: Connection) => (
    <div key={c.id}>
    ...
    <div className="flex flex-col flex-1 min-h-0">
        ...
        <div className="px-5 pt-4 pb-0 shrink-0">
          <div className="relative">
            ...
            <input
              ...
              placeholder={t("explorer.connectionsModal.searchPlaceholder")}
              className="w-full text-xs pl-7 pr-7 py-1.5 rounded border border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900/50 focus:outline-none focus:ring-1 focus:ring-azure-primary dark:text-zinc-200"
            />
            {search && (
              <button
                onClick={() => setSearch("")}
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 space-y-2">
        ...
        {search && filtered.length === 0 && (
          <p className="text-xs text-zinc-400 dark:text-zinc-500 py-2">
            {t("explorer.connectionsModal.noSearchResults", { query: search })}
          </p>
        )}
        ...
        {groups.map(({ key, items }) => (
          <div key={key || "__other"}>
            {showGroupHeaders && (
              <p className="text-[10px] font-semibold uppercase tracking-wider text-zinc-400 dark:text-zinc-500 mb-1.5 mt-1 first:mt-0">
                {key
                  ? t(`explorer.connectionsModal.env.${key}`, key)
                  : t("explorer.connectionsModal.groupOther")}
              </p>
            )}
            <div className="space-y-2">
              {items.map(renderConnectionItem)}
            </div>
          </div>
        ))}
